...
class DeRotationService(
        ...
        for (i in allImagesFilenames.indices) {
            ...
            if (referenceImageFilename == sourceImageFilename) {
                ...
                while (!targetAtReference) {
                    val targetImageFilename = if (sourceReachedReference)
                    ...
                    val targetImage = if (targetImageFilename == referenceImageFilename)
                        referenceImage
                    else
                        Opener().openImage("${rootFolder}/${targetImageFilename}")
            ...
            LswFileUtil.saveImage(
                sourceImage, null, "${derotationWorkFolder}/D_${sourceImageFilename}", true, false, false, false
            )
    ...
    private fun applyTransformation(sourceImage: ImagePlus, targetImage: ImagePlus, transformationFile: String) {
        for (layer in 1..sourceImage.stack.size) {
            ...
            bUnwarpJ_.applyTransformToSource(
                transformationFile, ImagePlus("Layer ${layer}", targetProcessor), sourceLayerImage
            )
    ...
    private fun createTransformationFiles(
        ...
        for (i in sharpenedImagePaths.indices) {
            ...
            if (referenceImageFilename == originalSource) {
                ...
                val transformationFile = callBunwarpJAlignImages(derotationWorkFolder, source, target, accurateness)
    ...
    private fun increaseProgressCounter(statusMessage: String) {
        ...
        if (luckyStackWorkerContext.isWorkerStopped) {
            ...
            throw DeRotationStoppedException("DeRotation was stooped")
        }
    }

    private fun callBunwarpJAlignImages(folder: String, source: String, target: String, accurateness: Int): String {
        ...
        val method = bUnwarpJ_::class.java.getDeclaredMethod("alignImagesCommandLine", Array<String>::class.java)
