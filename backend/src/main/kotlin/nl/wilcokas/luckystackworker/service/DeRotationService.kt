...
class DeRotationService(
    ...
    private fun validateTransformationResult(
        ...
        for (x in 0 until sourceProcessor.width) {
            for (y in 0 until sourceProcessor.height) {
                ...
                if (targetValue > 655) { // exclude dark pixels (less than 1% saturation) from the average calculation
        ...
        val averageValueDeviation = if (totalPixels > 0) abs(totalValueSource - totalValueTarget) / totalPixels else 0
    ...
    private fun createTransformationFiles(
        ...
        for (i in sharpenedImagePaths.indices) {
            ...
            increaseProgressCounter("Creating transformation for image $originalSource")
        }
        log.info("Done")
    ...
    private fun createPreSharpenedLuminanceCopies(
        ...
        for (imageFilename in allImagesFilenames) {
            ...
            increaseProgressCounter("Creating de-rotation mask for image $imageFilename")
        }
