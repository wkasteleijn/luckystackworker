...
public class LswImageProcessingUtil {
    ...
    public static void copyInto(
            final ImagePlus origin, final ImagePlus destination, Roi roi, Profile profile, boolean copyMinMax) {
        ...
        if (roi != null) {
            destination.setRoi(
                    (int) roi.getXBase(), (int) roi.getYBase(), (int) roi.getFloatWidth(), (int) roi.getFloatHeight());
        }
    }
    ...
    public static void copyMinMax(final ImagePlus origin, final ImagePlus destination) {
        for (int slice = 1; slice <= origin.getStack().size(); slice++) {
            ...
            destination
                    .getProcessor()
                    .setMinAndMax(
                            origin.getProcessor().getMin(),
                            origin.getProcessor().getMax());
    ...
    public static float[] rgbToHsl(
        ...
        if (mode == LSWSharpenMode.LUMINANCE) {
            ...
            luminance =
                    ((includeRed ? red : 0) + (includeGreen ? green : 0) + (includeBlue ? blue : 0)) / luminanceDivisor;
        } else {
    ...
    public static void convertLayersToColorImage(
            short[][] layers, ImagePlus image, boolean includeRed, boolean includeGreen, boolean includeBlue) {
    ...
    public static void copyLayers(
            LswImageLayers layersDto, ImagePlus image, boolean includeRed, boolean includeGreen, boolean includeBlue) {
