...
public class LswImageProcessingUtil {
  ...
  public static void copyInto(
      final ImagePlus origin,
      final ImagePlus destination,
      Roi roi,
      Profile profile,
      boolean copyMinMax) {
    ...
    if (copyMinMax && (profile.getBrightness() > 0 || profile.getContrast() > 0)
  ...
  public static void copyMinMax(final ImagePlus origin, final ImagePlus destination) {
    for (int slice = 1; slice <= origin.getStack().size(); slice++) {
      ...
      destination
          .getProcessor()
          .setMinAndMax(origin.getProcessor().getMin(), origin.getProcessor().getMax());
  ...
  public static float[] rgbToHsl(
    ...
    if (mode == LSWSharpenMode.LUMINANCE) {
      float luminanceDivisor =
          (includeRed ? 1 : 0) + (includeGreen ? 1 : 0) + (includeBlue ? 1 : 0);
      luminance =
          ((includeRed ? red : 0) + (includeGreen ? green : 0) + (includeBlue ? blue : 0))
              / luminanceDivisor;
    } else {
  ...
  public static float[] hslToRgb(
      float hue, float saturation, float luminance, float hueCorrectionFactor) {
  ...
  public static boolean validateImageFormat(
      ImagePlus image, JFrame parentFrame, String activeOSProfile) {
    String message =
  ...
  public static short convertToShort(long value) {
    return (short)
        (value >= Constants.SHORT_HALF_SIZE ? value - Constants.UNSIGNED_INT_SIZE : value);
  }
  ...
  public static void setNonPersistentSettings(Profile profile, double scale, String openImageMode) {
    profile.setDispersionCorrectionEnabled(
        false); // dispersion correction is not meant to be persisted.
  ...
  public static void convertLayersToColorImage(
      short[][] layers,
      ImagePlus image,
      boolean includeRed,
      boolean includeGreen,
    ...
    int channelIndex1 =
        includeRed && includeGreen && includeBlue ? 0 : includeRed ? 0 : includeGreen ? 1 : 2;
    int channelIndex2 =
        includeRed && includeGreen && includeBlue ? 1 : includeRed ? 0 : includeGreen ? 1 : 2;
    int channelIndex3 =
        includeRed && includeGreen && includeBlue ? 2 : includeRed ? 0 : includeGreen ? 1 : 2;
  ...
  public static void copyLayers(
      LswImageLayers layersDto,
      ImagePlus image,
      boolean includeRed,
      boolean includeGreen,
      boolean includeBlue) {
  ...
  public static ImagePlus create16BitRGBImage(
    ...
    short[] redPixels;
    short[] emptyPixels =
        new short[unprocessedImageLayers.getWidth() * unprocessedImageLayers.getHeight()];
    ...
    ImageStack stack =
        new ImageStack(unprocessedImageLayers.getWidth(), unprocessedImageLayers.getHeight());
   ...
   */
  public static int preventBackgroundFromLightingUp(
      double oldValue, double newValue, double lowestValue, boolean preserveDarkBackground) {
  ...
  public static ImagePlus crop(ImagePlus image, Roi roi, String filepath) {
    ...
    return create16BitRGBImage(
        filepath, getLswImageLayers(newPixels, width, height), true, true, true);
  }

  public static LswImageLayers getLswImageLayers(short[][] pixels, int width, int height) {
    return LswImageLayers.builder().layers(pixels).width(width).height(height).build();
  }
  ...
  public static FloatProcessor createDeringMaskFloatProcessor(
      double radius, double threshold, ImageProcessor ip) {
    ...
    int average =
        (int)
            ((maxValue - minValue)
                / threshold); // start cutting of from threshold times the average.
    for (int position = 0; position < maskPixels.length; position++) {
      int value = LswImageProcessingUtil.convertToUnsignedInt(maskPixels[position]);
      if (value < average) {
