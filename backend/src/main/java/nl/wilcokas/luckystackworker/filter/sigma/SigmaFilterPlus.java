...
public class SigmaFilterPlus {
  ...
  public void applyDenoise1(ImagePlus image, Profile profile) {
    ...
    BigDecimal factor =
        profile.getDenoise1Amount().compareTo(new BigDecimal("100")) > 0
            ? new BigDecimal(100)
    ...
    factor =
        profile.getDenoise1AmountGreen().compareTo(new BigDecimal("100")) > 0
            ? new BigDecimal(100)
    ...
    factor =
        profile.getDenoise1AmountBlue().compareTo(new BigDecimal("100")) > 0
            ? new BigDecimal(100)
  ...
  private void doFiltering(
    ...
    for (int y = roi.y; y < roi.y + roi.height; y++) {
      ...
      for (int x = roi.x, p = x + y * width, xCache0 = kRadius; x < xEnd; x++, p++, xCache0++) {
        ...
        if (fullCalculation) {
          fullCalculation =
              smallKernel; // for small kernel, always use the full area, not incremental
          // algorithm
        ...
        double mean =
            sums[0]
                / kNPoints; // sum[0] is the sum over the pixels, sum[1] the sum over the squares
        ...
        if (count >= minPixNumber) pixels[p] = (float) (sum / count);
          ...
          if (outlierAware)
            pixels[p] =
                (float) ((sums[0] - value) / (kNPoints - 1)); // assumes that the current pixel is
          // an outlier
   ...
   * squares
   */
  private void getAreaSums(
      float[] cache, int cacheWidth, int xCache0, int[] lineRadius, int kSize, double[] sums) {
    double sum = 0, sum2 = 0;
  ...
  private synchronized void makeKernel(double radius) {
    if (radius >= 1.5
        && radius < 1.75) // this code creates the same sizes as the previous RankFilters
    radius = 1.75;
