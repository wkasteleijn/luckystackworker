...
/**
 * This plugin-Filter provides a selective mean (averaging) filter. In contrast
 * to the standard mean filter, it preserves edges better and is less sensitive
 * to outliers. Based on Lee's sigma filter algorithm and a plugin by Tony
 * Collins. J.S. Lee, Digital image noise smoothing and the sigma filter, in:
 ...
 * standard deviation of the pixel values within the neighborhood ("Use pixels
 * within ... sigmas"). If the number of pixels in this range is too low (less
 * than "Minimum pixel fraction"), averaging over all neighboring pixels is
 * performed. With the "Outlier Aware" option, averaging over all neighboring
...
public class SigmaFilterPlus {
     ...
     *                   2*kRadius+1
     * @param lineRadius The radius of the lines in the kernel. Line length of line
     *                   i is 2*lineRadius[i]+1. Note that the array
     *                   <code>lineRadius</code> will be modified, thus call this
     *                   method with a clone of the original lineRadius array if the
     *                   array should be used again.
     */
    //
    ...
    /**
     * Get sum of values and values squared within the kernel area. xCache0 points
     * to cache element equivalent to current x coordinate. Output is written to
     ...
     * left border outside the kernal area. Output is added or subtracted to/from
     * array sums[0] += sum; sums[1] += sum of squares when at the right border,
     ...
     * neighbors of the pixel in the center, radius = 1 corresponds to a 3x3 kernel
     * size. The output is written to class variables kNPoints (number of points
     * inside the kernel) and lineRadius, which is an array giving the radius of
     * each line. Line length is 2*lineRadius+1.
     */
