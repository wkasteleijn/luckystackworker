            ...
            CompletableFuture.runAsync(
                () -> {
                  for (int i = 0; i < redPixels.length; i++) {
                    ...
                    double rawValue =
                        LswImageProcessingUtil.convertToUnsignedInt(
                            unprocessedImageLayers.getLayers()[1][i]);
                    redPixels[i] =
                        LswImageProcessingUtil.convertToShort(
                            (long)
                                (value * (1 - blendRawRedFactor) + (rawValue * blendRawRedFactor)));
                  }
                },
                executor);
        futures[1] =
                ...
                () -> {
                  for (int i = 0; i < greenPixels.length; i++) {
                    ...
                    greenPixels[i] =
                        LswImageProcessingUtil.convertToShort(
                            (long)
                                (value * (1 - blendRawGreenFactor)
                                    + (rawValue * blendRawGreenFactor)));
                  }
                },
                executor);
        futures[2] =
                ...
                () -> {
                  for (int i = 0; i < bluePixels.length; i++) {
                    ...
                    bluePixels[i] =
                        LswImageProcessingUtil.convertToShort(
                            (long)
                                (value * (1 - blendRawBlueFactor)
                                    + (rawValue * blendRawBlueFactor)));
                  }
                },
                executor);
