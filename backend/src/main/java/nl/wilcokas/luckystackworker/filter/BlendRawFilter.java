...
/**
 * Note: experimental feature. Does not seem to work any better than applying it directly from the
 * LSWSharpenFilter.
 */
@Slf4j
@Component
public class BlendRawFilter {
    ...
    public void apply(
            ImagePlus image,
            final LswImageLayers unprocessedImageLayers,
            double blendRawRedFactor,
            double blendRawGreenFactor,
            double blendRawBlueFactor) {
                ...
                futures[0] = CompletableFuture.runAsync(
                        () -> {
                            for (int i = 0; i < redPixels.length; i++) {
                                ...
                                double rawValue = LswImageProcessingUtil.convertToUnsignedInt(
                                        unprocessedImageLayers.getLayers()[1][i]);
                                redPixels[i] = LswImageProcessingUtil.convertToShort(
                                        (long) (value * (1 - blendRawRedFactor) + (rawValue * blendRawRedFactor)));
                            }
                        },
                        executor);
                futures[1] = CompletableFuture.runAsync(
                        () -> {
                            for (int i = 0; i < greenPixels.length; i++) {
                                ...
                                double rawValue = LswImageProcessingUtil.convertToUnsignedInt(
                                        unprocessedImageLayers.getLayers()[2][i]);
                                greenPixels[i] = LswImageProcessingUtil.convertToShort(
                                        (long) (value * (1 - blendRawGreenFactor) + (rawValue * blendRawGreenFactor)));
                            }
                        },
                        executor);
                futures[2] = CompletableFuture.runAsync(
                        () -> {
                            for (int i = 0; i < bluePixels.length; i++) {
                                ...
                                double rawValue = LswImageProcessingUtil.convertToUnsignedInt(
                                        unprocessedImageLayers.getLayers()[2][i]);
                                bluePixels[i] = LswImageProcessingUtil.convertToShort(
                                        (long) (value * (1 - blendRawBlueFactor) + (rawValue * blendRawBlueFactor)));
                            }
                        },
                        executor);
