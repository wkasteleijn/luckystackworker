...
public class LSWSharpenFilter implements LSWFilter {
  ...
  public void applyRGBModeClippingPrevention(
      ImagePlus image, final UnsharpMaskParameters unsharpMaskParameters) {
  ...
  public void applyLuminanceMode(ImagePlus image, final LSWSharpenParameters parameters) {
    ...
    for (int it = 0; it < unsharpMaskParameters.getIterationsLuminance(); it++) {
      ...
      if (unsharpMaskParameters.getDeringStrengthLuminance() > 0.0f) {
        ...
        final FloatProcessor fpMask =
            LswImageProcessingUtil.createDeringMaskFloatProcessor(
  ...
  public void applyLuminanceModeClippingPrevention(
      ImagePlus image, final LSWSharpenParameters parameters) {
    if (!parameters.isIncludeRed() && !parameters.isIncludeGreen() && !parameters.isIncludeBlue()) {
      log.error("Cannot have red, green and blue excluded");
      return;
    }
  ...
  private void doUnsharpMaskClippingPrevention(
    ...
    for (int y = roi.y; y < roi.y + roi.height; y++) {
      for (int x = roi.x, p = width * y + x; x < roi.x + roi.width; x++, p++) {
        int cutoffIndex =
            Math.round(
                ((initialPixels[p] < 0f ? 0f : initialPixels[p]) / FLOAT_MAX_SATURATED_VALUE)
                    * 100f);
        ...
        float pixelValueNew =
            getUnsharpMaskValue(snapshotPixels[p], pixels[p], amountNew, blendRawFactor);
  ...
  private void doUnsharpMaskDeringing(
    ...
    for (int y = roi.y; y < roi.y + roi.height; y++) {
      for (int x = roi.x, p = width * y + x; x < roi.x + roi.width; x++, p++) {
        ...
        float cutoffFactor =
            (Constants.MAX_INT_VALUE - maskValue) / (Constants.MAX_INT_VALUE / 100);
  ...
  private float getUnsharpMaskValue(
      float pixelValue, float pixelValueAfterBlur, float amount, float blendRawFactor) {
