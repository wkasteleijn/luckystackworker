 ...
 * @author Piotr Wendykier (piotr.wendykier@gmail.com) Adapted by Wilco Kasteleijn for use in
 *     LuckyStackWorker.
 */
@Slf4j
public class LswWPLFloatIterativeDeconvolver2D {
  ...
  /** If true, then the convergence information is displayed after each iteration. */
  ...
  /** This parameter is used to stop the iteration if the image is not changing. */
  protected float changeThreshPercent;

  /** If true, then all the data is in decibels. */
  protected boolean dB;

  /** If true then the iterations are stopped when the changes appear to be increasing. */
   ...
   */
  public LswWPLFloatIterativeDeconvolver2D(
      ImagePlus imB,
      ImagePlus imPSF,
    ...
    sum = PSF.zSum();
    if ((sum != 0) && this.normalize) scalePSF /= sum;
  ...
  public ImagePlus deconvolve() throws ExecutionException, InterruptedException {
    ...
    if (threshold == -1.0) {
      ...
      FloatCommon2D.assignPixelsToProcessorPadded(
          ip, X, bRows, bColumns, rOff, cOff, cmY, threshold);
    }
  ...
  private void convolveFD(
      final int rows, final int columns, FloatMatrix2D H1, FloatMatrix2D H2, FloatMatrix2D Result)
  ...
  private void copyDataAverage(
      final int rows,
      final int columns,
      final int rowsE,
      final int columnsE,
      final float sum,
      FloatMatrix2D DataIn,
      FloatMatrix2D DataOut,
  ...
  private float energySum(
      FloatMatrix2D X, final int rows, final int columns, final int cOff, final int rOff) {
    float sumPixels = 0;
  ...
  private float findMagMax(FloatMatrix2D H2) {
    ...
    if ((np > 1) && (rows * columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      ...
      magMax = results[0];
      for (int j = 1; j < np; j++) {
        if (results[j] > magMax) magMax = results[j];
      }
    } else {
      int cC, rC, idx1, idx2;
      float mag;
  ...
  private void gaussianFilter(FloatMatrix2D X, final float[][] weights)
      throws ExecutionException, InterruptedException {
  ...
  private void gaussianFilterWithScaling(
      FloatMatrix2D X, final float[][] weights, final float scale)
      throws ExecutionException, InterruptedException {
    final float[] elems = (float[]) X.elements();
    final int rows = X.rows();
    final int columns = X.columns();
  ...
  private float[][] gaussianWeights(
      final int rows, final int columns, final float filterX, final float filterY) {
              ...
              CompletableFuture.runAsync(
                  () -> {
                    for (int c = firstCol; c < lastCol; c++) {
                      int cShifted = c;
                      if (cShifted > columns / 2) cShifted = columns - cShifted;
                      float tmp = (cShifted / cc);
                      weights[0][c] = (float) Math.exp(-tmp * tmp);
                    }
                    for (int r = firstRow; r < lastRow; r++) {
                      int rShifted = r;
                      if (rShifted > rows / 2) rShifted = rows - rShifted;
  ...
  private void toDB(FloatMatrix2D X, final float minDB)
      throws ExecutionException, InterruptedException {
