...
public class ClippingSuppressionFilter implements LSWFilter {
  ...
  private void apply(ImagePlus image, double clippingSuppression) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        long scaledRed =
            (long) (red[index] * getRelativeGainFactor(red[index], clippingSuppression));
        long scaledGreen =
            (long) (green[index] * getRelativeGainFactor(green[index], clippingSuppression));
        long scaledBlue =
            (long) (blue[index] * getRelativeGainFactor(blue[index], clippingSuppression));
        redPixels[index] =
            LswImageProcessingUtil.convertToShort(
                scaledRed > MAX_INT_VALUE ? MAX_INT_VALUE : scaledRed);
        greenPixels[index] =
            LswImageProcessingUtil.convertToShort(
                scaledGreen > MAX_INT_VALUE ? MAX_INT_VALUE : scaledGreen);
        bluePixels[index] =
            LswImageProcessingUtil.convertToShort(
                scaledBlue > MAX_INT_VALUE ? MAX_INT_VALUE : scaledBlue);
      }
    }
  }
  ...
  private double getRelativeGainFactor(double pxValue, double clippingSuppressionFactorPercentage) {
    ...
    return (((MAX_DOUBLE_VALUE - pxValue) / MAX_DOUBLE_VALUE) * suppressionFactor)
        + (1D - suppressionFactor);
  }
}
