    ...
    private void apply(ImagePlus image, double clippingSuppression) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                redPixels[index] =
                        LswImageProcessingUtil.convertToShort(scaledRed > MAX_INT_VALUE ? MAX_INT_VALUE : scaledRed);
                greenPixels[index] = LswImageProcessingUtil.convertToShort(
                        scaledGreen > MAX_INT_VALUE ? MAX_INT_VALUE : scaledGreen);
                bluePixels[index] =
                        LswImageProcessingUtil.convertToShort(scaledBlue > MAX_INT_VALUE ? MAX_INT_VALUE : scaledBlue);
            }
        }
    }
