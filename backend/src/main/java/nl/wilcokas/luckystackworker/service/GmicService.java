  ...
  public void callGmicCli(
    ...
    try {
      ...
      LswFileUtil.saveImage(
          image, profileName, inputFile, image.getStack().size() > 1, false, false, false);
      String outputFile = workFolder + "/temp_out.tif";
      List<String> arguments =
          new ArrayList<>(Arrays.asList("v", "2", "-input", inputFile, "-div", "65536"));
      ...
      ImagePlus outputImage =
          LswFileUtil.openImage(
                  LswFileUtil.getIJFileFormat(outputFile), OpenImageModeEnum.RGB, scale, img -> img)
              .getLeft();
