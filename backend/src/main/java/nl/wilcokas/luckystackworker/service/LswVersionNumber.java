...
public class LswVersionNumber {
    ...
    public static Optional<LswVersionNumber> fromString(String version, String body) {
        String[] versionString = version != null && version.length() > 2 && version.contains(".") ? version.split("\\.") : null;
        if (versionString != null && versionString.length == 3) {
            ...
            if (body != null) {
                String releaseNotesString = body.contains("## Release notes") ? body.substring(body.indexOf("## Release notes") + 16) : StringUtils.EMPTY;
                releaseNotes = new ArrayList<>(Arrays.asList(
                        releaseNotesString.replaceAll("-", "")
                                .replaceAll("\\r", "")
                                .split("\\n"))).stream().map(String::trim).filter(StringUtils::isNotEmpty).toList();
            }
            return Optional.of(LswVersionNumber.builder()
                    .major(majorVersion)
                    .minor(minorVersion)
                    .patch(patchVersion)
