...
public class WorkerService {
  ...
  public void doWork() {
    try {
      ...
      if (luckyStackWorkerContext.isProfileBeingApplied()) {
        ...
        if (luckyStackWorkerContext.isRealTimeEnabled()
            && luckyStackWorkerContext.isRootFolderSelected()) {
  ...
  private Collection<File> getImages(boolean recursive) {
    return FileUtils.listFiles(
        Paths.get(settingsService.getRootFolder()).toFile(),
        settingsService.getExtensions(),
        recursive);
  }
