	...
	public String sendHttpGetRequest(HttpClient.Version httpVersion, String url, int timeoutSeconds) {
		...
		try {
			...
			if (response.statusCode() == 200) {
				...
				log.warn("Unable to do HTTP request, got the following response code from server: {}",
						response.statusCode());
			}
		} catch (Exception e) {
			log.warn("Unable to do HTTP request, got the following response code and message from server: {}, {}",
					response == null ? null : response.statusCode(), e.getMessage());
		}
		return null;
