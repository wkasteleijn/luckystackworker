...
public class FilterService {
  ...
  public byte[] applyAllFilters(
    ...
    int cacheWidth =
        cache.isEmpty() ? 0 : cache.entrySet().stream().findFirst().get().getValue().getWidth();
    int cacheHeight =
        cache.isEmpty() ? 0 : cache.entrySet().stream().findFirst().get().getValue().getHeight();
  ...
  private FilterEnum getPreviousCachedOperation(
      Profile profile, ImagePlus image, List<FilterEnum> operations) {
  ...
  private byte[] updatePSF(
      PSF psf, List<FilterEnum> operations, String profileName, boolean isMono) {
